pfGridMap = function(PF, bins, id=NULL, site.dat=NULL,
  ...
  require(paleofire) #   ?paleofiresites
  require(GCD)
  require(rgdal)         # Takes care of transformations, etc.
  require(raster)        # A well-developed library for raster data. Helped in plotting.
  require(gstat)         # a major geospatial analysis library (does the kriging)
  require(automap)       # A small library that automatically fits variograms
  require(rworldmap)     # A library of base maps
  require(RColorBrewer)  # A library of color palettes
  ...
  # Coordinate reference systems. Assuming WGS84 for GCD lat/lon. Some options for projection (besides default) are Mercator ("+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs") or Gall-Peters ("+proj=cea +lon_0=0 +lat_ts=45 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_def"). These "proj4" strings can be found online (e.g. http://www.spatialreference.org/)
  ...
  cat("Preparing data...")
    ...
    # Convert data to spatial data into a SpatialPointsDataFrame (useful for doing the spatial stuff below)
      ...
      # Specify the columns that are coordinates, and then the coordinate ref. system. This converts the data frame to class 'SpatialPointsDataFrame', a special class in the 'sp' library. 
  ...
  cat("Defining grid...")
    ...
    if(proj.map) {   # Gall-peters (units are meters)
      ...
      if(is.null(max.kr.dist)) 
        # Default maximum distance for kriging. Here, chosen to equal the distance from the center to a corner of each grid cell.
    ...
    # Unprojected (units are degrees, except max.kr.dist which is great-circle dist in km)
      ...
      if(is.null(max.kr.dist)) 
        # Default maximum distance for kriging. Here, a tricky choice since grid is defined in degrees but this parameter is in km. 
        max.kr.dist = 500
    }
  ...
  # Open file connection, if saving the plot
    ...
    # Parameters
      ...
      # Colors (uses an add-on library of color palettes). First argument is # of levels, then name of palette. rev() function switches its direction (the result of brewer.pal(...) is just a vector of color codes)
