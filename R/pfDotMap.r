...
pfDotMap = function(TR, bins, 
             ...
             proj4="+proj=longlat", n.boot=1000,
             cx.minsize=0.3, cx.mult=1
             ) {

# ---------------- TEST BLOCK
# Easier to test without running the code as a function. Comment everything above here (function definition) and uncomment this code to run the whole thing as a standard script. The load() line needs to point to where a pre-made pfTransform object has been saved. You will also need to load the two additional functions at the end of this file (just run them once at the beginning of each session). And technically, don't run the '}' that closes the main function definition (though I think if you do it will run everything and just give a harmless error at the end.
# 
# rm(list=ls())
# 
...
# fig.base.name     = '/Users/ryan/Drive/GPWG_MapPaper/Krige Maps/Dot Maps v2.1/Dot Maps v2.1'
# base.map          = 'coasts'
# grd.res           = 5
# grd.ext           = c(-180,180,-90,90)
# proj4             = "+proj=longlat"  # Corrsponds to unprojected maps
...
if(class(TR)=="pfTransform") {
  cat("Creating composite...")
    # Run pfComposite. Not interested in the composite, but this will do the binning for us. (I've checked "by hand" and it is accurate and efficient.)
...
# ----- Get lat/lon from GCD
  ...
  # A little awkward, but seems good to rely on existing paleofire functions for retrieving site coordinates, and as far as I can tell this is the way to do it at present.
...
#  As discussed at AGU (Marlon, Bartlein, Higuera, Kelly), a simple way is to search within a radius around each grid cell center, equal to the greatest distance from a grid cell center to its most distant corner. This should occur at the equator, where grid cells are largest. Note that this conservative approach will result in many sites falling within multiple grid boxes--even at the equator, the defined circles will overlap near the edges of the grid boxes. At higher latitudes, the grid cells are much smaller, so overlap will be considerably greater. There are alternatives, like using a grid that is irregular in terms of lat/lon, or changing the area of grid cells depending on latitude. But all have their tradeoffs (we thought), and this one is simple. 
...
#       cat(j,"...",sep="")
      ...
      sp.grd = cbind(grd.lonlat, grd.n[,j], grd.mean[,j], grd.lCI[,j], grd.uCI[,j])
        ...
        proj4string(sp.grd) = "+proj=longlat"
      ...
      sp.sites = data.frame(sites.lon, sites.lat, dat.n.contributions[,j])
        ...
        proj4string(sp.sites) = "+proj=longlat"

      # Now project the data to be plotted
      sp.grd     = spTransform(sp.grd, CRS(proj4))
      sp.sites   = spTransform(sp.sites, CRS(proj4))

      # Get x/y lims (could add option to override later). Only purpose currently is that sites vs. grid have different bounding boxes, which is the default extent for spplot(). I.e. without setting all the same, the bottom-right plot will have different extent than other two.
      x.lim = bbox(sp.grd)[1,]
      y.lim = bbox(sp.grd)[2,]
    ...
    # ----- Plot Number of sites per grid cell
      # Generate dot sizes and corresponding key. 
        ...
        # Define sizes of data points and legend entries by dividing data into bins and scale to range [0,1]. This is a pretty good range for symbol 'cex' sizes, although can be modified with the cx.mult and cx.minsize arguments. 'cut(..., labels=F)' returns integer classes from 1:n.cx. These are the same sizes to use in the key. 
...
# ---------- Auxilliary functions
  # Haversine distance function (http://en.wikipedia.org/wiki/Haversine_formula)
    ...
    # Note: at least on pair of inputs (i.e., either (lon1,lat1) or (lon2/lat2)) must represent a single location. The other may be vectors representing many locations, but the function does not work to find all pairwise distances between two sets (n>1) of locations. It wouldn't be difficult to code the pairwise version, but I think it wouldn would require a loop so it wouldn't save any run time anyway.
  ...
  # Bootstrap CI function.
    # Actually should work for various functions and CI probs but this code currently only relies on the defaults (mean, 95% CI).
